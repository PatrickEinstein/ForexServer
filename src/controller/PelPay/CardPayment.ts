...
const CardPaymentPelPay: RequestHandler = async (req, res, next) => {
  try {
    ...
    const response = await fetch(
      `https://api.pelpay.ng/Payment/process/card/${adviceRef.responseData.adviceReference}`,
      {
        method: "POST",
        headers: {
    ...
    console.log(
      `creq==>`,
      deliveredResponse.responseData.formData.formData.JWT,
      `acsUrl==>`,
      deliveredResponse.responseData.formData.url
    );
